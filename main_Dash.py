...
app.layout = html.Div([
    ...
    html.Table([
        ...
        html.Tr([html.Td('ipTkns Cost'), html.Td(id='iptkns_cost')]),
    ]),
])
...
def update_output_div(input_value, input_value2, input_value3, input_value4, input_value5):
    ...
    def objective(x, sign=-1.0):
        ...
        for t in np.arange(0, no_tkns):
            if user_ip_tkns[t] == 0:
                ...
                apr = fn.staking_pool(new_stkd_amts[t], new_pwripor_list[t], ipor_prc, vrt_shft, base_boost,
                                      log_base,
    ...
    b = (
        min(user_pwripor, user_pwripor + (change / ipor_prc)),
    ...
    return pwrtk_chng, pwripor_final, ipor_cost, ipor_cost_final
